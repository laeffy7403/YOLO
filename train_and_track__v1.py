...
def train_yolo(epochs=EPOCHS):
    ...
    print(f"[INFO] Training complete in {mins} min {secs} sec.")
    print(f"[INFO] Weights saved to {MODEL_PATH}")
...
def track_from_screen(model):
    with mss.mss() as sct:
        monitor = sct.monitors[1]  # Full screen (monitor[0] = all, [1] = primary)
...
if __name__ == "__main__":
    ...
    # Step 3: Repeating tracking loop
    ...
    print(f"\n🕒 Total Runtime (Training + All Tracking): {total_mins} min {total_secs} sec")
